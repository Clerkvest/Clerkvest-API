...
public class PostCommentResourceTest {
    ...
    public void postCommentAsSelf() {
        ProjectDTO project0 = given().header("X-API-Key", "exampleToken0").get(HateoasLink.PROJECT_SINGLE + "0").then().statusCode(OK.value()).extract().as(ProjectDTO.class);
    ...
    public void postCommentAsEmployee() {
        EmployeeDTO employee1 = given().header("X-API-Key", "exampleToken1").get(HateoasLink.EMPLOYEE_SINGLE + "1").then().statusCode(OK.value()).extract().as(EmployeeDTO.class);
        ProjectDTO project0 = given().header("X-API-Key", "exampleToken0").get(HateoasLink.PROJECT_SINGLE + "0").then().statusCode(OK.value()).extract().as(ProjectDTO.class);
    ...
    public void postCommentAsForeign() {
        EmployeeDTO employee3 = given().header("X-API-Key", "exampleToken3").get(HateoasLink.EMPLOYEE_SINGLE + "3").then().statusCode(OK.value()).extract().as(EmployeeDTO.class);
        ProjectDTO project0 = given().header("X-API-Key", "exampleToken0").get(HateoasLink.PROJECT_SINGLE + "0").then().statusCode(OK.value()).extract().as(ProjectDTO.class);
